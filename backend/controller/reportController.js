...
const exportTasksReport = async (req, res) => {
    try {
        const tasks =await Task.find({ role: { $ne : "admin"}}).populate("assignedTo", "name email");

        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet("Tasks Report");

        worksheet.columns = [
            { header: "No.", key: "index", width: 25},
...
const exportUsersReport = async (req, res) => {
    try {
        const users = await User.find({ role: { $ne: "admin" }}).select("name email _id").lean();
