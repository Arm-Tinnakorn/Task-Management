import React from 'react' 
import moment from 'moment'
import { useNavigate } from 'react-router-dom';

const TaskListTable = ({tableData, role}) => {
    const navigate = useNavigate()

    const handleNavigate = (task) => {
  if (role === 'admin') {
    navigate('/admin/create-task', { state: { taskId: task._id } });
  } else {
    navigate(`/user/task-details/${task._id}`);
  }
};


    const getStatusBadgeColor = (status) => {
        switch (status) {
            case 'Completed' : return 'bg-green-200 text-green-500 border border-green-500 border-2 rounded-full'
            case 'Pending' : return  'bg-purple-200 text-purple-500 border border-purple-500 border-2 rounded-full'
            case 'In Progress' : return 'bg-cyan-200 text-cyan-500 border border-cyan-500 border-2 rounded-full'
            default: return 'bg-gray-100 text-gray-500 border border-gray-500 border-2 rounded-full';
        }
    };

    const getPriorityBadgeColor = (priority) => {
        switch (priority) {
            case 'High' : return 'bg-red-200 text-red-500 border-2 border-red-500 rounded-full '
            case 'Medium' : return 'bg-yellow-200 text-orange-500 border-2 border-orange-500 rounded-full '
            case 'Low' : return 'bg-green-200 text-green-500 border-2 border-green-500 rounded-full'
            default: return 'bg-gray-100 text-gray-500 border border-gray-200'
        }
    }
  return (
    <div className='overflow-x-auto p-0 rounded-lg mt-3 bg-red-300'>
        <table className='min-w-full'>
            <thead>
                <tr className='text-left'>
                    <th className='py-3 px-4 text-gray-800 font-medium text-[13px]'>Name</th>
                    <th className='py-3 px-4 text-gray-800 font-medium text-[13px]'>Status</th>
                    <th className='py-3 px-4 text-gray-800 font-medium text-[13px]'>Priority</th>
                    <th className='py-3 px-4 text-gray-800 font-medium text-[13px] hidden md:table-cell'>Created On</th>
                </tr>
            </thead>
            <tbody className=''>
                {tableData.map((task) => (
                    <tr key={task._id} className='border-t border-gray-200 cursor-pointer' onClick={() => handleNavigate(task)}>
                        <td className="my-3 mx-4 text-gray-700 text-[13px] line-clamp-1 overflow-hidden" >{task.title}</td>
                        <td className="py-4 px-4">
                            <span className={`px-2 py-1 text-xs rounded inline-block ${getStatusBadgeColor(task.status)}`}>{task.status}</span>
                        </td>
                        <td className="py-4 px-4">
                            <span className={`px-2 py-1 text-xs rounded inline-block ${getPriorityBadgeColor(task.priority)}`}>{task.priority}</span>
                        </td>
                            <td className="py-4 px-4 text-gray-700 text-[13px] text-nowrap hidden md:table-cell">{task.createdAt ? moment(task.createdAt).format('Do MMM YYYY') : 'N/A'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
    </div>
  )
}

export default TaskListTable
